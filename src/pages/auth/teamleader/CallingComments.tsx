...
const CallingComments: React.FC = () => {
    ...
    API.postData("get/my/parent/info", {
      ...
      .then((response: any) => {
        ...
        if (response.data.status == 1) {
          callingCommentList(0, response.data.manager_info.user_uid);
  ...
  const callingCommentList = (index: any, manager_uid: any) => {
    const techno_calling_admin = JSON.parse(
      localStorage.getItem("techno_calling_admin") || "{}"
    );
    setcurrentManagerUID(manager_uid);
    API.postData("get/all/beforecallingcomment/list?page=" + index, {
      access_key: techno_calling_admin.access_key,
      ...
      .then((response: any) => {
        console.log(response.data);
        if (response.data.status == 1) {
          setcommentList(response.data.beforecallingcomment_list);
          const total = response.data.beforecallingcomment_list.length / 10;
          setpageCount(total);
        }
      })
      .catch((error: any) => {
  ...
  const callingCommentList2 = (index: any) => {
    const techno_calling_admin = JSON.parse(
      localStorage.getItem("techno_calling_admin") || "{}"
    );
    API.postData("get/all/beforecallingcomment/list?page=" + index, {
  ...
  const handlePageClick = (event: any) => {
    callingCommentList2(event.selected);
  };
  const changeStatus = (e: any, callingcomment_uid: any) => {
    var status = "";
    if (e.target.value == "1") {
      status = "active";
    } else {
    ...
    API.postData("change/beforecallingcomment/status", {
      access_key: techno_calling_admin.access_key,
      beforecallingcomment_uid: callingcomment_uid,
      manager_uid: currentManagerUID,
      status: status,
    })
      .then((response: any) => {
                  ...
                  <div className="col-6">
                    <h2 className="welcome-title mt-4 mb-3">
                      ...
                      <button
                        ...
                        onClick={() => navigate.push("/add-calling-comment")}
                      >
                        <svg
...
export default CallingComments;
