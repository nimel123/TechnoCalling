...
const CallingPitch: React.FC = () => {
    ...
    API.postData("get/all/callingpitch/list?page=" + index, {
      ...
      .then((response: any) => {
        ...
        if (response.data.status == 1) {
          ...
          const total = response.data.callingpitch_list.length / 10;
          setpageCount(total);
        } else {
          setcallingList([]);
          setpageCount(0);
  ...
  const changeStatus = (e: any, callingpitch_uid: any) => {
    var status = "";
    if (e.target.value == "1") {
      status = "active";
    } else {
    ...
    API.postData("change/callingpitch/status", {
      access_key: techno_calling_admin.access_key,
      manager_uid: currentManagerUID,
      callingpitch_uid: callingpitch_uid,
      status: status,
    })
      .then((response: any) => {
        console.log(response.data);
  ...
  const getCatList = () => {
    ...
    API.postData("get/all/callingpitch/list?page=0", {
      access_key: techno_calling_admin.access_key,
      manager_uid: currentManagerUID,
      category_uid: currentCategoryUID,
      sub_category_uid: currentSubCategory,
    })
      .then((response: any) => {
        console.log(response.data);
        if (response.data.status == 1) {
                  ...
                  <div className="col-6">
                    <h2 className="welcome-title mt-4 mb-3">
                      ...
                      <button
                        ...
                        onClick={() => navigate.push("/add-calling-pitch")}
                      >
                        <svg
            ...
            <IonRow style={{ padding: "10px" }}>
              ...
              <IonCol size="4">
                ...
                <input
                  type="text"
                  readOnly
                  defaultValue={currentProfile?.pitch}
                  className="rec-select2 mt-2"
                />
              </IonCol>
            </IonRow>
