...
const AssignPeopleToCaller: React.FC = () => {
  ...
  const [currentCaller, setcurrentCaller] = useState<any>("");
  ...
  const formSchema1 = Yup.object().shape({
    ...
    caller_uid: Yup.string(),
    people_count: Yup.string().required("Number of people is required."),
  });
  ...
  const onSubmit1 = handleSubmit1((data: any) => {
    if (data.people_count > unassigned_people) {
      ...
      API.uploadFile("assign/people/to/caller", data)
        .then((response: any) => {
  ...
  const usersList = (index: any) => {
    ...
    API.postData("get/all/user/list?page=" + index, {
      ...
      .then((response: any) => {
        console.log(response.data);
        if (response.data.status == 1) {
          setteamLeader(response.data.user_list.data);
        } else {
          setteamLeader([]);
        }
      })
  ...
  const selectTeamleader = (e: any, index: any) => {
    ...
    API.postData("get/caller/list?page=" + index, {
      access_key: techno_calling_admin.access_key,
      ...
      .then((response: any) => {
        console.log(response.data);
        if (response.data.status == 1) {
          setcallerList(response.data.caller_list);
        } else {
          setcallerList([]);
        }
      })
  ...
  const selectCaller = (e: any, index: any) => {
    setcurrentCaller(e.target.value);
    setValue1("caller_uid", e.target.value);
    const techno_calling_admin = JSON.parse(
      localStorage.getItem("techno_calling_admin") || "{}"
    );
              ...
              <div className="container-fluid px-4">
                ...
                <div className="row">
                  ...
                  <div className="col-3">
                    <select
                      className="rec-select2 mt-2"
                      onChange={(e) => selectCaller(e, 0)}
                    >
                      <option value="">Please select caller</option>
                      {callerList.length > 0 &&
                        callerList.map((item: any, key: any) => (
                          <option key={key} value={item.uid}>
                            {item.fname} {item.lname}
                          </option>
                        ))}
                ...
                {currentSubCategoryUID && currentCaller && (
...
export default AssignPeopleToCaller;
