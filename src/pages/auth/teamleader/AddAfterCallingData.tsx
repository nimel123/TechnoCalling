...
const AddAfterCallingData: React.FC = () => {
  ...
  const onSubmit1 = handleSubmit1((data: any) => {
    API.postData("add/aftercallingcomment", data)
      .then((response: any) => {
  ...
  const getParentRole = () => {
    ...
    API.postData("get/my/parent/info", {
      ...
      .then((response: any) => {
        console.log(response.data);
        if (response.data.status == 1) {
          setValue1("manager_uid", response.data.manager_info.user_uid);
        } else {
        }
      })
      .catch((error: any) => {
...
export default AddAfterCallingData;
