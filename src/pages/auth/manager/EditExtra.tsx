...
const EditExtra: React.FC = () => {
  ...
  const onSubmit1 = handleSubmit1((data: any) => {
    API.postData("update/extracolumn", data)
      .then((response: any) => {
  ...
  useEffect(() => {
    ...
    setValue1("column_uid", uid);
    API.postData("get/extracolumn", {
      ...
      .then((response: any) => {
        ...
        if (response.data.status == 1) {
          setValue1(
            "column_name",
            response.data.column_info.peopleextracolumns_name
          );
        } else {
          toast.error(response.data.message);
        }
