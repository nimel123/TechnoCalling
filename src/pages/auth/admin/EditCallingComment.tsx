...
const EditCallingComment: React.FC = () => {
  ...
  const formSchema1 = Yup.object().shape({
    ...
    beforecallingcomment_uid: Yup.string(),
  ...
  const onSubmit1 = handleSubmit1((data: any) => {
    API.postData("update/beforecallingcomment/info", data)
      .then((response: any) => {
  ...
  useEffect(() => {
    ...
    setValue1("beforecallingcomment_uid", id);
    setValue1("manager_uid", uid);
    API.postData("get/beforecallingcomment/info", {
      access_key: techno_calling_admin.access_key,
      manager_uid: uid,
      beforecallingcomment_uid: id,
    })
      .then((response: any) => {
        console.log(response.data);
        if (response.data.status == 1) {
          setValue1("comment", response.data.beforecallingcomment_info.comment);
        } else {
          toast.error(response.data.message);
        }
      })
...
export default EditCallingComment;
